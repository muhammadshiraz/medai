...
#def expose_services(app, api, project_dir, swagger_host: str, PORT: str):
        ...
        try:        
            ...
            if clz_name == "Patient" and method in ["POST","PATCH"]:
                ...
                if patient_id is not None and "insulinData" in data and len(data["insulinData"]) > 0:
                    insertInsulin(data['insulinData'], patient_id=patient_id)
    ...
    def insertInsulin(insulinData, patient_id):
        
        for insulin in insulinData: 
            id = insulin.get("id", None)
            updateRow = None
            if id is not None:
                # update existing insulin
                updateRow = session.query(models.Insulin).filter(
                    models.Insulin.id == id,
                    models.Insulin.patient_id == patient_id,
                ).one_or_none()
            ...
            drug = insulin["drug"] if "drug" in insulin and insulin["drug"] is not None else ''
            drug_id = getDrugId(drug) 
            row.drug_type = drug_id
    ...
    def insertMedication(data, patient_id):
        for medication in data["medications"] : 
            ...
            drug = medication["drug"] if "drug" in medication and medication["drug"] is not None else 'Lispro'  
            drug_id = getDrugId(drug)         
            reading_values = {}
            '''
            ...
            '''
            id = medication.get("id", None)
            updateRow = None
            if id is not None:
                updateRow = session.query(models.PatientMedication).filter(
                    models.PatientMedication.id == id,
                    models.PatientMedication.patient_id == patient_id,
                ).one_or_none()

            row = models.PatientMedication() if updateRow is None else updateRow
            row.patient_id = patient_id
            ...
            for time_of_reading in ['breakfast', 'lunch', 'dinner']:
                ...
                reading_values[time_of_reading] = value if value != '-' else '0'
                setattr(row, time_of_reading, reading_values[time_of_reading])
            session.add(row)
            try:
                session.commit()
                session.flush()  
    ...
    def updateReading(readings, patient_id):
        for reading in readings : 
            id = reading["id"] if "id" in reading and reading["id"] is not None else None
            reading_row = session.query(models.Reading).filter(models.Reading.id == id).one_or_none() if id is not None else None
            ...
            try:
                ...
                app_logger.error(f"Error updating reading for patient {patient_id}: {ex}")
    def insertReading(readings, patient_id):
        for reading in readings : 
            updateRow = None
            if "id" in reading and reading["id"] is not None:
                # update existing reading
                updateRow = session.query(models.Reading).filter(models.Reading.id == reading["id"]).one_or_none()
            reading_row = updateRow if updateRow is not None else models.Reading()
            reading_row.patient_id = patient_id
